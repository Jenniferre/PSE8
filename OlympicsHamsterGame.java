...
public final class OlympicsHamsterGame extends SimpleHamsterGame {
	...
	private void recordAttempt() {
		...
		final FeedingStrategy twiceStrategy = new FeedTwiceStrategy();
		speedy.setFeedingTactics(twiceStrategy);
		speedy.setRacePlan(new SprinterRacePlan());
		while (!speedy.hasFinished()) {
			if (speedy.getActionsTaken() == 10) {
	...
	/**
	 * sets up the runners to take part in a race with strategies as implemented in
	 * task (c)
	 * 
	 * @ensures a steady runner and a sprinting runner both on the tile (1,1) and
	 *          facing east have been added to the runners list with their
	 *          respective race strategies
	 */
	private void setupTaskC() {
		...
		final RunnerHamster sprintingRunner = new RunnerHamster(game.getTerritory(), new Location(1, 1),
				Direction.EAST);
	 ...
	 * @ensures a steady runner and a sprinting runner both on the tile (1,1) and
	 *          facing east have been added to the runners list with their
	 *          respective race and feeding strategies
	 */
